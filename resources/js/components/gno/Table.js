...
export default {
  ...
  methods: {
    ...
    postCurveData() {
      ...
      if(this.isYoungAge ==false &&(this.pResInput.split(' ')[0] * 1 <= this.bhpInput.split(' ')[0] * 1 || this.pResInput.split(' ')[0] * 1 <= this.bhpCelValue.split(' ')[0] * 1)) {
        this.$notify({
        ...
        if(this.expChoose == 'ШГН'){
          ...
          if(this.isVisibleChart) {
            ...
            this.axios.post(uri, this.postdata).then((response) => {
              ...
              if (this.isYoungAge==false) {this.fetchBlockCentrators()}
              if(data) {
