...
export default {
  name: "economic-data-empper-component",
  components: {
    VueTableDynamic,
    CatLoader,
    SelectScFa
  ...
  methods: {
    async getData() {
      ...
      const {data} = await this.axios.get(this.localeUrl('/eco_refs_emp_pers'), {params: this.form})
  ...
  computed: {
    params() {
      return {
        ...
        headerHeight: 120,
        rowHeight: 50,
        columnWidth: this.columns.map((col, index) => ({column: index, width: 250})),
        fontSize: this.columns.map((col, index) => ({column: index, fontSize: 16}))
      }
    },
