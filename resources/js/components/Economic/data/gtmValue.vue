...
export default {
  name: "economic-data-gtm-value-component",
  components: {
    DeleteButton
  },
  data: () => ({
  ...
  methods: {
    async getData() {
      ...
      try {
        const {data} = await this.axios.get(this.localeUrl('/eco_refs_gtm_values'))
    ...
    async deleteGtm(id) {
      try {
        await this.axios.delete(this.localeUrl(`/eco_refs_gtm_value/${id}`))
