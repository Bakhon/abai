  ...
  <div class="filter-container">
    ...
    <div class="form-group1 filter-group select">
      ...
      >
        ...
        <option value="Ввод из бездействия силами ПРС">Ввод из бездействия силами ПРС</option>
        <option value="Изоляция водопритоков с применением новых изолирующих составов">Изоляция водопритоков с применением новых изолирующих составов</option>
        <option value="Изоляция воды">Изоляция воды</option>
        <option value="Кислотная обработка (КСПЭО)">Кислотная обработка (КСПЭО)</option>
        <option value="Кислотная обработка (Колтюбинг)">Кислотная обработка (Колтюбинг)</option>
        <option value="Кислотная обработка призабойной зоны пласта">Кислотная обработка призабойной зоны пласта</option>
        <option value="Освоение скважины">Освоение скважины</option>
        <option value="Перевод на механизированную добычу">Перевод на механизированную добычу</option>
        <option value="Ревизия и замена УЭЦН и ШГН">Ревизия и замена УЭЦН и ШГН</option>
        <option value="Ремонтно-изоляционные работы">Ремонтно-изоляционные работы</option>
        <option value="Реперфорация">Реперфорация</option>
        <option value="Соляно-кислотная обработка">Соляно-кислотная обработка</option>
        ...
        <option value="Дострел">Дострел</option>
        <option value="Перфорация (прострел)">Перфорация (прострел)</option>
        <option value="Переход на другой объект">Переход на другой объект</option>
        <option value="Перфорация (перестрел)">Перфорация (перестрел)</option>
        <option value="Гидроимпульсное воздействие">Гидроимпульсное воздействие</option>
        <option value="Перфорация (дострел)">Перфорация (дострел)</option>
        <option value="Гидроразрыв пласта">Гидроразрыв пласта</option>
        <option value="Возврат (переход) на вышележащие/нижележащие горизонты">Возврат (переход) на
          вышележащие/нижележащие горизонты
        </option>
        <option value="Освоение после ГРП">Освоение после ГРП</option>
      </select>
    </div>
      ...
      <button
        ...
        <span>
          <svg width="16" height="20" viewBox="0 0 16 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                d="M14.7066 5.70711L10.2925 1.2929C10.105 1.10536 9.85061 1 9.58539 1L1.99979 1.00002C1.44751 1.00002 0.999797 1.44774 0.999797 2.00002L0.999797 18C0.999797 18.5523 1.44751 19 1.9998 19L13.9995 19C14.5518 19 14.9995 18.5523 14.9995 18L14.9995 6.41421C14.9995 6.14899 14.8942 5.89464 14.7066 5.70711Z"
                stroke="white" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M9.11768 1V6.02353C9.11768 6.57581 9.56539 7.02353 10.1177 7.02353H14.9999" stroke="white"
...
export default {
  ...
  methods: {
    ...
    updateData() {
      let uri = process.env.MIX_MICROSERVICE_PREDEFINED_REPORTS + "/analyze/gtm/";

      let data = {
        period: 'days',
        dzo: this.org,
        report_date_start: formatDate.getMinOfDayFormatted(this.start_date),
        report_date_end: formatDate.getMinOfDayFormatted(this.end_date),
        report_date: formatDate.getMinOfDayFormatted(this.report_date),
