...
export default {
  name: "economic-data-macro-component",
  components: {
    VueTableDynamic,
    CatLoader,
    SelectScFa
  ...
  methods: {
    async getData() {
      ...
      const {data} = await this.axios.get(this.localeUrl('/eco_refs_macros'), {params: this.form})
  ...
  computed: {
    params() {
      return {
        ...
        headerHeight: 120,
        rowHeight: 50,
        columnWidth: this.columns.map((col, index) => ({column: index, width: 200}))
      }
    },
    ...
    columns() {
      ...
      return [
        {value: 'source_data'},
        {value: 'month-year'},
        {value: 'course_prices', dimension: 'tenge'},
        {value: 'course_rubles', dimension: 'tenge'},
        {value: 'inf_end'},
        {value: 'add', dimension: 'dollar'},
      ]
    },
  }
};
