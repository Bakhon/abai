      ...
      <div class="col-12 blueblock one">
        <div class="d-flex">
          ...
          <div class="blueblock__buttons d-flex flex-wrap scrollable">
            <div v-for="button in filteredFormButtons" class="blueblock__buttons-button" @click="loadForm(button.code)">
              <div class="d-flex flex-row justify-content-center block-one-icon" v-html="button.ico">
              </div>
              <div class="block-one-title text-center" v-html="button.name"></div>
            </div>
          </div>
        </div>
      </div>
...
<script>
import BigDataForm from './wells/form'
import BigDataTable from './wells/table'
...
export default {
  data() {
    return {
      forms: {
        well_register: {
          code: 'well_register',
          title: 'Регистрация новой скважины',
          type: 'form'
        },
        fluid_production: {
          code: 'fluid_production',
  ...
  computed: {
    filteredFormButtons() {
      ...
      if (this.formNameQuery) {
        return this.buttons.filter(button => button.name.toLowerCase().indexOf(this.formNameQuery.trim().toLowerCase()) > -1)
      }
  ...
  mounted() {
    ...
    this.axios.get('/js/json/bd/forms.json').then(({data}) => {
      this.buttons = data
    })
  },
  methods: {
