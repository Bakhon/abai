<template>
    <table class="table table-bordered table-dark table-responsive ce" style="position: sticky;left: 5.31%;right: 2.4%;top: 48.21%;bottom: 66.58%;background: #0D1E63;">
        ...
        <tr v-for="(row, row_index) in wells" :key="row_index">
            ...
            wells[row_index].well[1][0] !== '0'}">
                <span class="circle-err" :style="`background :${getColor(
            wells[row_index].well[1][0])}`"> </span>
                <span>{{row.well[0]}}</span>
                <span v-if="wells && wells[row_index]" class="cell-comment">
                    {{ wells[row_index].well[1][1]}}
                </span>
            </td>
            ...
            <td :class="{'cell-with-comment': wells && wells[row_index] &&
            wells[row_index].h_up_perf_vd[1][0] !== '0'}">
                <span class="circle-err" :style="`background :${getColor(
            wells[row_index].h_up_perf_vd[1][0])}`"> </span>
                <span>{{row.h_up_perf_vd[0]}}</span>
                <span v-if="wells && wells[row_index]" class="cell-comment">
                    {{ wells[row_index].h_up_perf_vd[1][1]}}
                </span>
            </td>
            ...
            <td :class="{'cell-with-comment': wells && wells[row_index] &&
            wells[row_index].EMPTY[1][0] !== '0'}">
                <span class="circle-err" :style="`background :${getColor(
            wells[row_index].EMPTY[1][0])}`"> </span>
                <span>{{Math.round(row.EMPTY[0]*10)/10}}</span>
                <span v-if="wells && wells[row_index]" class="cell-comment">
                    {{ wells[row_index].EMPTY[1][1]}}
                </span>
            </td>
