...
class OmgCAHistoryObserver extends EditHistoryObserver
{
    ...
    private function generatePayload(OmgCA $omgCA): array
    {
        ...
        foreach ($fields as $field => $name) {
            switch ($field) {
                case 'year':
                    $oldValue = Carbon::parse($original['date'])->format('Y');
                    $newValue = isset($changes['date']) ? Carbon::parse($changes['date'])->format('Y') : $oldValue;
                    break;
                case 'gu_id':
                    $oldValue = $original['gu_id'] ? Gu::find($original['gu_id'])->name : null;
                    $newValue = isset($changes['gu_id']) ? Gu::find($changes['gu_id'])->name : $oldValue;
                    break;
                default:
