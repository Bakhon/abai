    ...
    protected function sort(string $field, bool $isDescending)
    {
        switch($field) {
            ...
            case 'water_type_by_sulin':
                ...
                ->orderBy('water_type_by_sulins.name', $isDescending ? 'desc' : 'asc');
                break;
            case 'sulphate_reducing_bacteria':
                ...
                ->orderBy('sulphate_reducing_bacterias.name', $isDescending ? 'desc' : 'asc');
                break;
            case 'hydrocarbon_oxidizing_bacteria':
                ...
                ->orderBy('hydrocarbon_oxidizing_bacterias.name', $isDescending ? 'desc' : 'asc');
                break;
            case 'thionic_bacteria':
                $this->query
                ->select('water_measurements.*')
                ->leftJoin('thionic_bacterias', 'thionic_bacterias.id', 'water_measurements.thionic_bacteria_id')
                ->orderBy('thionic_bacterias.name', $isDescending ? 'desc' : 'asc');
                break;
            default:
